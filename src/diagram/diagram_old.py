...
class BasicDiagram(AbstractDiagram):
    
    
    def __init__(self, scale = 1, supScale = 0.8):
        """
        
        The diagram manages the style of how structural elements.
        
        A plotter handles all the style of plotting.
        There could be different types of diagram classes with different styles

        
        It can handle abstract systems, and could be applied to frame elements
    ...
    def plotPointForce(self, x, y, dx, dy, baseWidth = 0.03, c='C0'):
        """
        
        Makes an arrow that points from x/y to the destination point x+dx, y+dy
        Note, Px and Py must be normalized!
        https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.arrow.html
        ...
        hwidth = width* 5
        length = width* 5
        self.ax.arrow(x, y, dx, dy, width=width, head_width = hwidth, head_length = length, 
                      edgecolor='none', length_includes_head=True, fc=c)
    ...
    # TODO: look into why we have this?! It doesn't do anything?
    def _get_ystart(self, q, y0):
         
        if q < 0: # if the direction is negative, start high and point down
    ...
    def plotVerticalLinearLoad(self, x1:float, x2:float, q:list[float],
                                ydatum:float = 0, spacing:float = 1, 
                                barC:str = 'grey'):
        """
        ...
        # The bars on either side of the chart.
        xbar  = [x1, x2]
        yBarS = [ydatum, ydatum]                     
        yBarE = [ydatum - q1, ydatum - q2]
        ...
        for ii in range(Nlines):
            ...
            if abs(yLine) > minLengthCutoff:
                self.plotPointForce(xline, ydatum - yLine, 0, yLine, 
                                    baseWidth = baseLineWidth, c = barC)
                
            # plot just the line with no arrow
            else:
                width = baseLineWidth*self.scale
                self.ax.plot([xline, xline], [ydatum - yLine, ydatum], c = barC,
                              linewidth=width)
