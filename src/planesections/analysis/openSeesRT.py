...
try:
    ...
    raise Exception('OpenSees has not been installed. First include optional depependancy with "pip -m install planesections[opensees]"')


from .recorder import OutputRecorder

class OutputRecorderOpenSeesRT(OutputRecorder):
    """
    ...
    """
    def __init__(self, runtime, beam:bb.Beam):
        ...
        for ii, node in enumerate(beam.nodes):
            ID = node.ID
            node.disps = np.array(self._runtime.nodeDisp(ID))
            node.rFrc  = np.array(self._runtime.nodeReaction(ID))
    ...
    def getEleInteralForce(self, nodID:int):
        ...
        if nodID == self.nodeID0: # Left most node
            ...
            Fint[ndf:] =  self._runtime.eleForce(eleR)[:ndf]             # Right side forces

        #Direct Check, this is scary.
        elif nodID == self.nodeIDEnd: # right side node
            eleL = nodID - 1
            Fint[:ndf] = -np.array(self._runtime.eleForce(eleL)[ndf:]) # Right side forces
            Fint[ndf:] = 0                               # Left side forces
        else: # center nodes
            eleL = nodID - 1
            eleR = nodID

            Fint[:ndf] = -np.array(self._runtime.eleForce(eleL)[ndf:]) # left entries
            Fint[ndf:] =  np.array(self._runtime.eleForce(eleR)[:ndf]) # right entries
        
        return Fint


class OutputRecorder2D(OutputRecorderOpenSeesRT):
...
class OpenSeesAnalyzer2D:
    ...
    """
    def __init__(self, beam2D:bb.Beam, recorder = OutputRecorderOpenSeesRT,
                 geomTransform = 'Linear', clearOld = True):

        self.beam:bb.Beam = beam2D
        self._checkBeam(beam2D)

        # create an independent analysis runtime
        self._runtime = op.Model('Basic' , ndm=2, ndf=3)

        self.Nnode = beam2D.Nnodes
        self.Nele = self.Nnode - 1
        self.recorder = partial(recorder, self._runtime)
    ...
    def initModel(self, clearOld = True):
        ...
        if clearOld:
            self._runtime.wipe()
        self._runtime.model('Basic' , '-ndm',  2)
        self._runtime.geomTransf(self.geomTransform, 1)
    ...
    def buildEulerBeams(self):
        ...
        for ii in range(self.Nele):
            ...
            self._runtime.element(beam.EleType, eleID , Ni, Nj, A, E, Iz, 1)
           
    
    def _buildPointLoads(self, pointLoads):
        for load in pointLoads:
            self._runtime.load(int(load.nodeID), *load.P)
    ...
    def buildPointLoads(self):
        ...
        """        
        self._runtime.timeSeries('Linear',1)
        self._runtime.pattern('Plain', 1, 1)
    ...
    def buildAnalysisPropreties(self):
        ...
        """
        # self._runtime.constraints("Transformation")
        self._runtime.constraints("Lagrange")
        self._runtime.numberer("Plain")
        self._runtime.system('BandGeneral')
        self._runtime.test('NormDispIncr',  1.*10**-8, 40, 0 , 2)
        # self._runtime.algorithm('Newton')
        self._runtime.algorithm('Linear')
        self._runtime.integrator('LoadControl',1.)
        self._runtime.analysis('Static')

    def analyze(self):
        """
        Analyzes the model once and records outputs.
        """
        ok = self._runtime.analyze(1)
        self._runtime.reactions()   
        return ok
    ...
    def buildEleLoads(self):
        ...
        """
        self._runtime.timeSeries('Linear',2)
        self._runtime.pattern('Plain', 2, 2)
    ...
    def _buildDistLoad(self, Nodes:list[int], eleload:bb.EleLoadLinear):
        ...
        for ii in range(N1, N2):                
            self._runtime.eleLoad('-ele', int(ii), 
...
class OpenSeesAnalyzer3D:
    ...
    def __init__(self, beam3D:bb.Beam, recorder = OutputRecorderOpenSeesRT,
                 geomTransform = 'Linear', clearOld = True):
        ...
        # create an independent analysis runtime
        self._runtime = op.Model('Basic' , ndm=3, ndf=6)
    ...
    def initModel(self, clearOld = True):
        ...
        if clearOld:
            self._runtime.wipe()
        self._runtime.model('Basic' , '-ndm',  3)
        # see https://opensees.berkeley.edu/wiki/index.php/PDelta_Transformation
        self._runtime.geomTransf(self.geomTransform, 1, *[0, 0, 1])
    ...
    def buildNodes(self):
        ...
        for node in self.beam.nodes:
            self._runtime.node(int(node.ID), float(node.x), 0., 0.)
            
            # OpenSees is very finicky with these inputs, int them for saftey.
            f1, f2, f3, f4, f5, f6 = node.fixity.fixityValues
            self._runtime.fix(node.ID, int(f1), int(f2), int(f3), int(f4), int(f5), int(f6))
    ...
    def buildEulerBeams(self):
        ...
        for ii in range(self.Nele):
            ...
            self._runtime.element(beam.EleType,  eleID , Ni, Nj , A, E, G, J, Iy, Iz, 1)
    ...
    def buildEleLoads(self):
        ...
        for eleload in self.beam.eleLoads:
            ...
            for ii in range(N1, N2):
                ...
                self._runtime.eleLoad('-ele', int(ii), '-type', '-beamUniform', load[1], load[2], load[0])   
