...
def test_Fnet():
    ...
    y = (R1+R2)[1]
    assert abs(y + x4*q[1][0]/2) < 0.0001
...
def test_Mmax():
    ...
    check1 = abs(Mmin - (q[1][0]*(x4-x3)**2/(9*3**0.5)))/abs(Mmin) -1 < tol
...
def test_Vmax():
    ...
    check1 = abs(Vmin - q[1][0]*x2/6) < tol
    check2 = abs(Vmax + q[1][0]*x2/3) < tol
...
def test_reversed():
    ...
    assert( abs(y1 + q[1][0]*x4/6) / abs(y1) - 1 < tol)
    assert( abs(y2 + q[1][0]*x4/3) / abs(y2) - 1< tol)
...
def test_FEM():
    ...
    R1, R2 = beam.reactions
    y = (R1+R2)[1]
    assert abs(y + (x4-x3)*q[1][0]/2) / abs(y)  < 0.0001
