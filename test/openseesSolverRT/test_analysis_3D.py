...
def test_node_loads_3D():
    ...
    assert np.sum(np.abs(pointBeamAnalysis._runtime.nodeReaction(3) - np.array([0,500,0,0,0,-625]))) < 0.001
...
def test_node_loads_3D_2():
    ...
    assert np.sum(np.abs(analysis3D_2._runtime.nodeReaction(1) - np.array([0,0,500,0,-625,0]))) < 0.001
...
def test_ele_loads_3D():
    ...
    assert np.sum(np.abs(distBeamAnalysis3D._runtime.nodeReaction(2) - solution)) < 0.001
...
def test_full_beam_3D():
    ...
    assert np.sum(np.abs(analysis3D._runtime.nodeReaction(3) - solution)) < 0.001
